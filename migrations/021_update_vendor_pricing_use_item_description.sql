-- Migration 021: Update vendor pricing view to use item_description for quotes
-- This provides more descriptive information in the vendor pricing grid

-- Drop the existing view
DROP VIEW IF EXISTS takeoff.v_current_vendor_pricing;

-- Recreate the view to use item_description for quotes
...
-- Quotes data using item_description from quotes view
SELECT 
    -q.quote_id as pricing_id,  -- Negative ID to distinguish from vendor_pricing
    v.vendor_name,
    v.vendor_id,
    vq.item_description AS product_description,  -- Use item_description from quotes view
    p.product_id,
    i.item_name,
    cc.cost_code,
    q.price,
...
JOIN takeoff.v_quotes vq ON q.quote_id = vq.quote_id  -- Join with quotes view to get item_description
...
'Combined view of current vendor pricing and quotes for vendor pricing grid with detailed item descriptions';
